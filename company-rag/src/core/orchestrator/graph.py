...
from src.core.orchestrator.state import OmniQueryState ## yaha import hua h to ab orchestrator kavi v state ko use kega isko use krke apna data dalega aur fir jb formetted response store ho jayega to fire return kr dega 
...
def build_orchestrator_graph(router, agent_registry, llm_provider) -> StateGraph:
    ...
    # Build graph
    graph = StateGraph(OmniQueryState) # yha ham sare nodes, edges, entry point, end point, conditional edges k sath sath state ko v define krte h 
